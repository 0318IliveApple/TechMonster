...
class BattleViewController: UIViewController {
    ...
    func startBattle() {
        ...
        enemyAttacTimer = Timer.scheduledTimer(timeInterval: enemy.attackInterval, target: self, selector: #selector(self.enemyAttack), userInfo: nil, repeats: true)
    ...
    func finishBattle(winPlayer:Bool) {
        ...
        enemyAttacTimer.invalidate()
        
        let finishedMassge: String
        if winPlayer == true {
            TechDraUtil.playSE(fileName: "SE_fanfare")
            finishedMassge = "プレーヤーの勝利！！"
        }else {
            TechDraUtil.playSE(fileName: "SE_gameover")
            finishedMassge = "プレーヤーの敗北..."
        }
        let alert = UIAlertController(title: "バトル終了", message: finishedMassge, preferredStyle: UIAlertControllerStyle.alert)
